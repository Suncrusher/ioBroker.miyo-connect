...
class MiyoStructureHandler {
    ...
    async handleDeviceData(deviceJSONData) {
        ...
        for (const deviceId in devices.data.params.devices) {
            ...
            for (let i = 0; i < keys.length; i++) {
                const key = keys[i];
                const attributeType = this.getAtrributeTypeByName(key);
                if (key != "stateTypes") {
            ...
            for (const stateTypesId in device.stateTypes) {
                ...
                const attributeType = this.getAtrributeTypeByName(stateType);
    ...
    async handleCircuiteData(deviceJSONData) {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if (key == "params" || key == "sensorValve") {
                    ...
                    for (let i = 0; i < cKeys.length; i++) {
                        ...
                        const attributeType = this.getAtrributeTypeByName(cKey);
...
exports.MiyoStructureHandler = MiyoStructureHandler;
//# sourceMappingURL=miyoStructureHandler.js.map